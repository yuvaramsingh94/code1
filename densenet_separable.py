...
def DenseNet(input_shape=None, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1, nb_layers_per_block=-1,
             ...
             classes=10, activation='softmax',non_local=False):
    ...
    x = __create_dense_net(classes, img_input, include_top, depth, nb_dense_block,
                           ...
                           dropout_rate, weight_decay, subsample_initial_block, activation,non_local=non_local)
...
def DenseNetImageNet121_non_local(input_shape=None,
                        bottleneck=True,
                        reduction=0.5,
                        dropout_rate=0.0,
                        weight_decay=1e-4,
    ...
    return DenseNet(input_shape, depth=121, nb_dense_block=4, growth_rate=32, nb_filter=64,
                    ...
                    dropout_rate=dropout_rate, weight_decay=weight_decay, subsample_initial_block=True,
                    include_top=include_top, weights=weights, input_tensor=input_tensor,
                    classes=classes, activation=activation,non_local=True)
...
def __conv_block(ip, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1e-4):
    ...
    x = SeparableConv2D(nb_filter, (3, 3), padding='same', use_bias=False)(x)
    if dropout_rate:
...
def __transition_up_block(ip, nb_filters, type='deconv', weight_decay=1E-4):
    ...
    if type == 'upsampling':
        ...
        x = SeparableConv2D(nb_filters, (3, 3), activation='relu', padding='same', 
...
def __create_dense_net(nb_classes, img_input, include_top, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
                       ...
                       subsample_initial_block=False, activation='softmax',non_local=False):
    ...
    x = SeparableConv2D(nb_filter, initial_kernel, kernel_initializer='he_normal', padding='same',
               strides=initial_strides, use_bias=False)(img_input)
    ...
    if include_top:
        ...
        print ("The activation used here is : ",activation)
...
def __create_fcn_dense_net(nb_classes, img_input, include_top, nb_dense_block=5, growth_rate=12,
    ...
    # Initial convolution
    x = SeparableConv2D(init_conv_filters, (7, 7),  padding='same', name='initial_conv2D',
